...
public class CookModify2 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_cook_modify2);
    ...
    private void setActivity(){
        ...
        for(int i = 0; i < menuData.getCooks().size(); i++){
            LinearLayout lin = new LinearLayout(CookModify2.this);
            lin.setLayoutParams(layoutParams2);
            lin.setOrientation(LinearLayout.HORIZONTAL);

            // 이름 텍스트뷰
            final TextView cookName = new TextView(CookModify2.this);
            cookName.setLayoutParams(layoutParams);
            cookName.setText(menuData.getCooks().get(i).getName());
            cookName.setTextSize(20);
            cookName.setTextColor(Color.BLACK);
            cookName.setTypeface(null, Typeface.BOLD);

            // 포지션 텍스트뷰
            final TextView posName = new TextView(CookModify2.this);
            posName.setLayoutParams(layoutParams);
            posName.setText("   [ "+menuData.getCooks().get(i).getPosition()+" ]    Break ");
            posName.setTextSize(20);
            posName.setTextColor(Color.BLACK);

            final Switch switchBtn = new Switch(CookModify2.this);

            if(menuData.getCooks().get(i).isBreaktime()){
